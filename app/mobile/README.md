...
The mobile app includes an iOS WidgetKit extension and an Android AppWidget. They
are generated by `plugins/withHomeScreenWidgets.js` during `expo prebuild` and
are included in EAS builds. The widget opens QuickEx when tapped and refreshes
its launch state every 30 minutes. Because widgets are native targets, Expo Go
cannot display them; use a development or internal EAS build and add QuickEx
from the device's widget picker.

The plugin also configures the iOS application-group entitlement required for
future shared wallet/activity snapshots.
